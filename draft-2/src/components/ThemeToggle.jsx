...
export default function ThemeToggle() {
  ...
  if (!mounted) return <div className="w-14 h-14" />; 
      ...
      <AnimatePresence mode="wait">
        {theme === 'dark' ? (
          ...
          <motion.div
            ...
            transition={{ duration: 0.4, ease: [0.16, 1, 0.3, 1] }}
          >
            <Sun size={20} style={{ color: 'var(--red)' }} />
          </motion.div>
        )}
      </AnimatePresence>
      
      {/* Imperial Decor */}
      <div className="absolute inset-0 rounded-full border border-red/0 group-hover:border-red/20 transition-all duration-700 scale-110 group-hover:scale-100 pointer-events-none" />
      <div className="absolute inset-[1px] rounded-full border border-gold/0 group-hover:border-gold/10 transition-all duration-1000 delay-100 scale-90 group-hover:scale-100 pointer-events-none" />
